...
const id = new Date();
console.log(id.toTimeString());

schedule.scheduleJob({ hour: 16, minute: 34 }, async () => {
  const html = await request.get("https://www.iedcr.gov.bd/");
  fs.writeFileSync("./test.html", html);

  //update pdf
  const $ = await cheerio.load(html);
  const blink = $("a").attr("href").trim();
  const flink = `https://www.iedcr.gov.bd/${blink}`;
  request(flink).pipe(fs.createWriteStream("d.pdf"));
  console.log("iedcr file updated" + Date.now());
});

async function main() {
  const corona = [];
  let $ = await cheerio.load(fs.readFileSync("./test.html"));
